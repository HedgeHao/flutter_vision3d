...
class _MyAppState extends State<MyApp> {
  ...
  void openglRender() {
    FlutterVision.openglRender().then((value) => {
          if (FlutterVision.openglIsRendering)
            {Future.delayed(const Duration(milliseconds: 1), openglRender)}
        });
  }
  ...
  Future<dynamic> update(MethodCall call) async {
    if (call.method == 'onInference') {
      Float32List output =
          await models[0].getTensorOutput(0, [28, 28, 5]) as Float32List;
      ...
      if (faces.isNotEmpty) {
        for (FaceInfo f in faces) {
          r.add(
              PositionedRect(f.x1, f.y1, f.x2 - f.x1, f.y2 - f.y1, Colors.red));
        }
      }
                    ...
                    children: [
                      ...
                      depthTextureId == 0
                          ...
                          : Container(
                              ...
                              width: 240,
                              height: 180,
                              child: Texture(textureId: depthTextureId)),
                      const SizedBox(width: 10),
                      irTextureId == 0
                          ? const SizedBox()
                          : Container(
                              ...
                              width: 240,
                              height: 180,
                              child: Texture(textureId: irTextureId)),
                      const SizedBox(width: 10),
                      cameraTextureId == 0
                          ? const SizedBox()
                          : Container(
                      ...
                      : Listener(
                          ...
                          child: Container(
                              ...
                              child: Transform.rotate(
                                  angle: 180 * pi / 180,
                                  child: Texture(textureId: openglTextureId)))),
                  ...
                  Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    TextButton(
                        onPressed: () async {
                          ...
                          await depthPipeline
                              .applyColorMap(OpenCV.COLORMAP_JET);
                          ...
                          await FlutterVision.videoScreenshot(
                              16, '/home/hedgehao/Desktop/test.jpg');
                    ...
                    TextButton(
                        onPressed: () async {
                          LipsPipeline depthPipeline = LipsPipeline(2);
                          await depthPipeline
                              .applyColorMap(Random().nextInt(10), at: 1);
                        },
                        child: const Text('Replace pipeline')),
                    TextButton(
                      onPressed: () async {
                        TFLiteModel model = await TFLiteModel.create(
                            '/home/hedgehao/test/cpp/tflite/models/efficientdet.tflite');
                        models.add(model);

                        LipsPipeline rgbPipeline = LipsPipeline(1);
                        await rgbPipeline.clear();
                        await rgbPipeline.resize(320, 320,
                            mode: OpenCV.INTER_CUBIC);
                        ...
                        await rgbPipeline.setInputTensorData(
                            model.index, 0, LipsPipeline.DATATYPE_UINT8);
                        await rgbPipeline.inference(model.index);

                        await FlutterVision.test();

                        Float32List outputBoxes = await model
                            .getTensorOutput(0, [25, 4]) as Float32List;
                        Float32List outputClass =
                            await model.getTensorOutput(1, [25]) as Float32List;
                        Float32List outputScore =
                            await model.getTensorOutput(2, [25]) as Float32List;

                        print('Class(raw):$outputClass');
                        List<String> classes = outputClass
                            .map((e) =>
                                e == 0 ? '' : COCO_CLASSES[e.toInt() - 1])
                            .toList();

                        print('Class:$classes');
                        print(
                            'Score:${outputScore.map((e) => e.toStringAsFixed(2)).toList()}');
                        print(
                            'Boxes:${outputBoxes.map((e) => e.toStringAsFixed(2)).toList()}');
                        ...
                        for (int i = 0; i < 5; i++) {
                          ...
                          double width =
                              texture_width * outputBoxes[i * 4 + 3] - x;
                          double height =
                              texture_height * outputBoxes[i * 4 + 2] - y;
                          r.add(
                              PositionedRect(x, y, width, height, Colors.red));
                        }
                    ...
                    TextButton(
                        onPressed: () async {
                          if (models.isEmpty) {
                            TFLiteModel model = await TFLiteModel.create(
                                'D:/test/190625_faceDetector_t1.tflite');
                            models.add(model);
...
class PositionedRect extends StatelessWidget {
  ...
  const PositionedRect(this.left, this.top, this.width, this.height, this.color,
      {Key? key})
      : super(key: key);
