...
class FvCamera {
  ...
  static Future<FvCamera?> create(String serial, CameraType type) async {
    ...
    Map<Object?, Object?> result = await FlutterVision3d.channel.invokeMethod('fvCameraOpen', {'serial': serial, 'cameraType': type.index});
  ...
  Future<void> close() async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraClose', {'serial': serial, 'cameraType': cameraType.index});
  }

  Future<bool> enableStream({int videoModeIndex = 7}) async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraConfigVideoStream', {'serial': serial, 'videoModeIndex': videoModeIndex, 'enable': true, 'cameraType': cameraType.index});
  }

  Future<bool> disableStream({int videoModeIndex = 7}) async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraConfigVideoStream', {'serial': serial, 'videoModeIndex': videoModeIndex, 'enable': false, 'cameraType': cameraType.index});
  }

  Future<void> enablePointCloud() async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraEnablePointCloud', {'serial': serial, 'enable': true, 'cameraType': cameraType.index});
  }

  Future<void> disablePointCloud() async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraEnablePointCloud', {'serial': serial, 'enable': false, 'cameraType': cameraType.index});
  }
  ...
  Future<bool> configure(int prop, List<double> value) async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraConfig', {'prop': prop, 'value': Float32List.fromList(value), 'serial': serial});
  }

  Future<int> getConfiguration(int prop) async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraGetConfiguration', {'prop': prop, 'serial': serial});
  }

  Future<bool> screenshot(int index, String path, {int? cvtCode}) async {
    return await FlutterVision3d.channel.invokeMethod('fvCameraScreenshot', {'index': index, 'path': path, 'cvtCode': cvtCode ?? -1, 'serial': serial});
  }

  Future<int> getOpenCVMat(int index) async {
    return await FlutterVision3d.channel.invokeMethod('fvGetOpenCVMat', {'index': index, 'serial': serial});
  }

  Future<Map<String, double>> getIntrinsic(int index) async {
    Map<dynamic, dynamic> map = await FlutterVision3d.channel.invokeMethod('fvGetIntrinsic', {'index': index, 'serial': serial});

    return <String, double>{'fx': map['fx'], 'fy': map['fy'], 'cx': map['cx'], 'cy': map['cy']};
  }

  Future<bool> enableRegistration(bool enable) async {
    int ret = await FlutterVision3d.channel.invokeMethod('fvEnableRegistration', {'enable': enable, 'serial': serial});
    return ret == 0;
  }

  Future<bool> pauseStream(bool pause) async {
    return await FlutterVision3d.channel.invokeMethod('fvPauseStream', {'pause': pause, 'serial': serial});
  }

  Future<List<String>> getVideoModes(int index) async {
    List<Object?> modes = await FlutterVision3d.channel.invokeMethod('ni2GetAvailableVideoModes', {'index': index, 'serial': serial});
    return modes.map((e) => e.toString()).toList();
  }

  Future<bool> setVideMode(int index, int mode) async {
    return await FlutterVision3d.channel.invokeMethod('ni2SetVideoMode', {'index': index, 'mode': mode, 'serial': serial});
  }

  Future<String> getCurrentVideoMode(int index) async {
    return await FlutterVision3d.channel.invokeMethod('ni2GetCurrentVideoMode', {'index': index, 'serial': serial});
  }

  Future<String> getSerialNumber() async {
    return await FlutterVision3d.channel.invokeMethod('fvGetSerialNumber', {'serial': serial});
  }

  Future<void> loadPresetParameter(String path) async {
    return await FlutterVision3d.channel.invokeMethod('rsLoadPresetParameters', {'serial': serial, 'path': path});
  }
  ...
  Future<List<int>> getDepthData(DepthType depthType, {int? x, int? y, int? width, int? height}) async {
    ...
    List<Object?> data = await FlutterVision3d.channel.invokeMethod('fvGetDepthData', {'serial': serial, 'index': depthType.index, 'x': x, 'y': y, 'roi_width': width, 'roi_height': height});
  ...
  Future<void> setCameraCrop({required int startX, required int startY, required int endX, required int endY}) async {
    await FlutterVision3d.channel.invokeMethod('fvSetCameraCrop', {'serial': serial, 'startX': startX, 'startY': startY, 'endX': endX, 'endY': endY});
  }
